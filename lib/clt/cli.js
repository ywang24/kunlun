...
var questions = [
  ...
  },
  {
    ...
    validate: function( value ) {
      var pass = value.match(/^[a-zA-Z_$][a-zA-Z._$0-9]*$/i);
      if (pass) {
        return true;
      } else {
...
if(argv._[0] === 'init') {
  ...
  Inquirer.prompt( questions, function( answers ) {
    ...
    Exec('npm install kunlun appium q moment request saucelabs config mocha testarmada-magellan testarmada-magellan-mocha-plugin --save', function (error, stdout, stderr) {
      if (error !== null) {
        console.log('npm install failed: ' + error);
        console.log('Please try kunlun init again');
      } else {
        console.log('\n================================================================');
        console.log('Basic Appium test framework is created successfully!');
